...
client = MongoClient("mongodb+srv://skillsda:skillsda123@skillsda.ij2jb.mongodb.net/<dbname>?retryWrites=true&w=majority")
db = client.get_database('Cybersecurity')
user = db.User
...
app.config['MAIL_USERNAME'] = 'skillsdacyberlab@gmail.com'
app.config['MAIL_PASSWORD'] = '$Cyber123'
...
def validate(user):
    ...
    if not any(x.isdigit() for x in user['password']):
        e+=1
        flash('Password should contain atleast one number!!','warning')
    if not user['password']==user['reppassword']:
        e+=1
        flash('Repeat Password does not match!!','warning')
    #check email
    regex = '^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$'
        ...
        if existing_user is None:
            if validate(request.form):
                ...
                if password==reppassword:
                    ...
                    types = 'Student'
                    user.insert_one({'username' : username, 'email':email, 'password' : hashpass, 'type':types})
                    session['logged_in'] = True
                    session['username'] = username
                    session['type'] = 'Student'
                    flash('You are now logged in','success')
...
@app.route('/challenge/<name>',methods=['GET'])
def challenge(name):
    # name =  
    return render_template('challenge.html',name=name)

@app.route('/predict/<name>',methods=['GET'])
def predict(name):
    loaded_model = pickle.load(open('linear_skillsda.sav', 'rb'))
    result = loaded_model.predict(np.array([4,4]).reshape(1, -1))
    print(result)
    time = round(result[0],2)
    flash('Predicted Time : '+str(time)+' hours to complete','success')
    return redirect('/challenge/'+name)
...
def faculty_dash():
    ...
    return render_template('dashboard_org.html',comments=comments)
